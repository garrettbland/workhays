 ...
 */
export const generateClasses = (calloutType = ''): string => {
    switch (calloutType) {
        case 'success': {
...
export const Callout = ({ type, icon, children }: CalloutProps) => {
    return (
        <p
            className={`px-3 py-1 flex flex-row items-center space-x-2 rounded border mb-6 max-w-2xl ${generateClasses(
                type
            )}`}
        >
            {icon ? icon : null}
